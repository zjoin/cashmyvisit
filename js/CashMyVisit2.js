...
var CMV_J=jQuery.noConflict( true );


function CMV_Call_Event(EventName)
{
	var z_CMV_ya_Counter="";
		var keys =Object.keys( window );
			for( var i = 0; i < keys.length; ++i ) 
			{
				...
				{
					z_CMV_ya_Counter=keys[ i ];
					if (z_CMV_ya_Counter.length)
					{
					try{
						z_CMV_ya_Counter=z_CMV_ya_Counter+".reachGoal('"+EventName+"');"
						eval(z_CMV_ya_Counter);
						}
						catch(e)
						{
	 ...
	 try{
				...
				{
					ga('send', 'event', 'CashMyVisit', EventName);
				}
		} catch(e) {}
...
var b3=0;
var v_move=0;
var v_scroll_total=0;
var v_scroll=0;
var v_scroll_bottom=0;
var v_focus=1;
var v_timer_focus=0;
var v_timer_move=0;
var v_total_timer=0;
...
function CMV_On_Move()
{
var prev_x=0;
var prev_y=0;
var v_move_y=0
var timer;
		CMV_J('body').mousemove(function(event){
			if (v_widget==1){return;}
			if (prev_x==0)
			{
				prev_x=event.pageX;
				prev_y=event.pageY;
			}
			if (prev_x!=event.pageX)
			{
				b1=b1+Math.abs(prev_x-event.pageX);
			}
			if (prev_y!=event.pageY)
			{
				b2=b2+Math.abs(prev_y-event.pageY);
			}
			
			
			v_move_y=event.pageY-CMV_J(window).scrollTop();;
			
			if (prev_y>event.pageY&&v_move_y<100)
			{
				v_move_up=1;
			}
			else
			...
			// CMV_Log(msg) ;
			// var msg2= " totalpath  x= "+b1+" y="+b2;
			// CMV_Log(msg2) ;
			//var msg2= " position  x= "+event.pageX+" y="+v_move_y;
			//CMV_Log(msg2) ;
			//CMV_Log("moveup="+v_move_up) ;
			 v_move=1;
			clearInterval(timer);
			timer = setTimeout(function() { v_move=0 ;}, 1000);

});

}

function CMV_On_Scroll()
{
var prev=0;

CMV_J(window).scroll(function () { 
		if (v_widget==1){return;}

		var pos= CMV_J(window).scrollTop();

		if (prev!=0)
		{
			v_scroll_total=parseInt(v_scroll_total+Math.abs(prev-pos));
		}
		prev=pos;
		
		if (Pars.A1=="0"&&Pars.A11=="50"&&v_scroll_bottom==0)
		{
			if( Math.abs( CMV_J(window).scrollTop() + CMV_J(window).height() -CMV_J(document).height())<10) {
				CMV_Log("Bottom!");
				v_scroll_bottom=1;
			}
		}
		...
		v_scroll=1;
		setTimeout(function() { v_scroll=0;//CMV_Log("scroll stopped")
		}, 1000);
...
function CMV_On_focus_and_Blur()
{
CMV_J(window).focus(function() { 
v_focus=1;
			CMV_Log("focus" +v_focus+" M1="+Pars.M1);
			var url_focus=z_CMV_Server+"/Call/Widget/Do?zzType=GetSettings_Focus";
			CMV_J.post(url_focus,{C1:Pars.C1,URL:window.location.href},function(data)
						{
							Pars = CMV_J.parseJSON(data);
						});


});

CMV_J(window).blur(function() { 
v_focus=0;
CMV_Settings_Blur();
...
function CMV_On_Entrance()
{
	if (Pars.A2=="0"&&Pars.A7=="1")
		{
			var timer1=setInterval(function()
			 {
				if 	(Pars.A2=="1"){clearInterval(timer1);return;}
			 
			 
			 CMV_Log("E timer"+Pars.A9+"<"+v_timer_move);
				if ( (parseInt(Pars.A9))<v_timer_move)
					{
					 CMV_Log("Entrance!");
						Pars.A2="1";
						clearInterval(timer1);
						var z_timeout_time=3;
							if (v_scroll==0)
								{
									z_timeout_time=3000;
								}
							setTimeout(function() 
								{
									//CMV_Call_Event("CMV_WidgetShowTimer");
									CMV_GetCallWidget("17");
								}, z_timeout_time);
...
function CMV_On_Leave()
{
	CMV_J('body').mouseleave(function() {
	CMV_Log("leave?");
		if (Pars.A8=="1"&&Pars.A6=="0"&&v_move_up==1&&Pars.A1=="0")
			{
				//if  ((parseInt(Pars.A3)+v_timer_move)>23)
				//{
					CMV_Log("CMV_On_Leave");
					Pars.A6="1";
					CMV_Settings_Blur();
					CMV_GetCallWidget("18");
				//}
		
			}
			
	
		else if (Pars.A6=="0"&&v_move_up==1&&Pars.A1=="0")
		{
			
			if  (Pars.M1=="1")
				{
					CMV_Log("CMV_On_Leave2");
					Pars.A6="1";
					CMV_Settings_Blur();
					CMV_GetCallWidget("18");
				}
...
function CMV_Timer_Total()
{
var timer1=setInterval(function()
			{
				v_total_timer=v_total_timer+1;
				
					if (v_move==1&&v_widget==0)
					{
						v_timer_move=v_timer_move+1;
					}
					if (v_focus==1&&v_widget==0)
					{
						v_timer_focus=v_timer_focus+1;
					}
					
					//CMV_Log("total timer="+v_total_timer+" v_timer_focus="+v_timer_focus+" v_timer_move="+v_timer_move+" v_scroll_total= "+v_scroll_total+ " move_total="+(b1+b2) );
...
function CMV_M_Timer()
{
if (Pars.M1=="0")
		{
			var timer1=setInterval(function()
			{
				if 	(Pars.M1=="1"){clearInterval(timer1);return;}
				
				if (parseInt(Pars.N1)<parseInt(Pars.T1)+v_timer_move)
				 {
					CMV_Set_M(1);
					clearInterval(timer1);return;
				 }
				 else if (parseInt(Pars.N2)<parseInt(Pars.T2)+v_timer_focus)
				 {
					CMV_Set_M(2);
					clearInterval(timer1);return;
				 }
				 else if (v_scroll_bottom==1)
				 {
					CMV_Set_M(4);
					clearInterval(timer1);return;
				 }
			...
			var timer1=setInterval(function()
			 {
				if (Pars.M1=="1")
				{
					if 	(Pars.A1=="1"){clearInterval(timer1);return;}
					if 	(Pars.A6=="1"){clearInterval(timer1);return;}
			 
					if (parseInt(Pars.P3)<parseInt(Pars.S3))
					{
						Pars.A1="1";
						clearInterval(timer1);
						CMV_Log("Show P3");
				...
				else if ( parseInt(Pars.P1)<v_timer_move+parseInt(Pars.S1))
					{
						Pars.A1="1";
						clearInterval(timer1);
						CMV_Log("Show P1");
				...
				else if ( parseInt(Pars.P2)<v_timer_focus+parseInt(Pars.S2))
					{
						Pars.A1="1";
						clearInterval(timer1);
						CMV_Log("Show P2");
...
function CMV_M_Show(delay1)
{
var z_timeout_time=3;
							if (v_scroll==1){z_timeout_time=3000;}
							if (delay1!=0){z_timeout_time=delay1;}
							CMV_Settings_Blur();
							CMV_Log('z_timeout_time='+z_timeout_time);
							setTimeout(function() 
								{
...
function CMV_Start()
{
		var cookie_val=CMV_readCookie("CMV_Z_Cookie");
		
		CMV_On_Scroll();
		CMV_On_Move();
		CMV_On_focus_and_Blur();
		CMV_On_Entrance();
		CMV_On_Leave();
		...
		var url_GetCallButton=z_CMV_Server+"/Call/Widget/Do?zzType=GetCallButton";
		CMV_J.post(url_GetCallButton,{GUID:cmv_trackerid,Cookie:cookie_val},function(data)
		{
				CMV_J('body').append(data);
				CMV_J('[cmv_widgetshow_button]').click(function (e) {
								var attr1=CMV_J(this).attr("cmv_next_dialog");
								CMV_GetCallWidget(attr1);
								CMV_Call_Event("CMV_WidgetShowButton");
							})	
		...
		CMV_J("[href='cashmyvisit#']").click(function (e) {
			e.preventDefault();
			CMV_Custom_Btn_Click(CMV_J(this));
		})
		CMV_J("[onclick='CMV_Custom_Btn_Click()']").click(function (e) {
			e.preventDefault();
			CMV_Custom_Btn_Click(CMV_J(this));
		})
		CMV_J("[onclick='CMV_Custom_Btn_Click();']").click(function (e) {
		...
		CMV_J("[CMV_Custom_Btn_Click]").click(function (e) {
			e.preventDefault();
			CMV_Custom_Btn_Click(CMV_J(this));
		})
		 ...
		 window.onbeforeunload = function() {                    
                    if (CMV_J('[cmv_widgetbody]').length)
						{
							CMV_Result(8);
						}
						CMV_Settings_Blur();
...
function CMV_Custom_Btn_Click(elem1)
{
			
			var cookie_val=CMV_readCookie("CMV_Z_Cookie");
			if (!CMV_J('[cmv_widgetbody]').length)
			{
				var cmv_custom_btn_name=CMV_J(elem1).attr('cmv_custom_btn_name');
				CMV_J.post(z_CMV_Server+"/Call/Widget/Do?zzType=GetRefButton",{GUID:cmv_trackerid,Cookie:cookie_val,cmv_custom_btn_name:cmv_custom_btn_name},function(data)
				{
						if (data!=="")
						{
							CMV_GetCallWidget(data);
						}
				})
			}	
}
...
function CMV_Set_M(elem1)
{
CMV_Log("CMV_Set_M  "+elem1);
CMV_Settings_Blur();

		setTimeout(function()
				{
					Pars.M1="1";
					var url_focus=z_CMV_Server+"/Call/Widget/Do?zzType=Set_M";
					CMV_J.post(url_focus,{C1:Pars.C1,M_Type:elem1},function(data)
						{
						Pars = CMV_J.parseJSON(data);
						});
				}
...
var CMV_LAST_WIDGET="777";
var CMV_LAST_WIDGETID="0";
function CMV_GetCallWidget(WidgetType)
{

	v_widget=1;
	

if (!WidgetType&&WidgetType!=0)
	{
		CMV_J('[cmv_widgetbody]').remove();
		//CMV_J('body>*').each(function( index, element ) {CMV_J(element).removeClass('cmv_blur');})
		return;
	}
	var cmv_widget_id="";
	...
	if (CMV_J('[cmv_widgetbody]').length)
		{
			cmv_widget_id=CMV_J('[cmv_widgetbody]').attr('cmv_widget_id');
		}
	if (CMV_LAST_WIDGET==WidgetType&&CMV_LAST_WIDGETID==cmv_widget_id)
	{
		return;
	}
	else
	{
		CMV_LAST_WIDGET=WidgetType;
	}

	var cookie_val=CMV_readCookie("CMV_Z_Cookie");
	if(cookie_val==null || cookie_val===false) {cookie_val="";}
	if(cmv_trackerid )
    {
		
		var Geo=CMV_J('[CMV_Custom_GEO]').text();
		var url_GetWidget=z_CMV_Server+"/Call/Widget/Do?zzType=GetWidget";
		CMV_J.post(url_GetWidget,{GUID:cmv_trackerid,WidgetID:cmv_widget_id,WidgetType:WidgetType,Cookie:cookie_val,From:document.referrer,Title:CMV_J(document).find("title").text(),URL:window.location.href,Geo:Geo},function(data){
				
			CMV_J('[cmv_widgetbody]').remove();
				CMV_J('body>*').each(function( index, element ) {
				
				if (!CMV_J(element).hasClass('cmv_phone'))
				{
					//CMV_J(element).addClass('cmv_blur');
				}
			...
			if (WidgetType=="18")
			{
				CMV_J('.cmv_main').css('top','0%');
				CMV_J(".cmv_main").animate({top:'10%'},450);
				//CMV_J(".cmv_main").animate({top:'45%'},450);
				//CMV_J(".cmv_main").animate({top:'50%'},450);
				//CMV_J(".cmv_main").animate({top:'49%'},450);
				//CMV_J(".cmv_main").animate({top:'50%'},400);
			}
			
			CMV_LAST_WIDGETID=CMV_J('[cmv_widgetbody]').attr('cmv_widget_id');
			...
			if (CMV_J.fn.jRating)
			{
				CMV_J('[cmv_widgetrating]').jRating({
				bigStarsPath : z_CMV_CDN_Server+'img/cmv_textmid__rate_star.png',
				step:true,length:5,decimalLength:0,rateMax:5,showRateInfo:false,sendRequest:false,canRateAgain : true,nbRates : 30,
			...
			if (CMV_J('[cmv_widgetdelay_select]').length)
				{
					CMV_J('[cmv_widgetdelay_select]').styler();
					CMV_Get_Call_Time();
					CMV_J('[cmv_widgetdelay_select]').change(function(){	
						CMV_Get_Call_Time();
						})
					
				}
				
				CMV_J('[cmv_widgetphone_input]').each(function() 
					{
					if (CMV_J.fn.mask)
							{				
								
								var jQuerythis = CMV_J(this);
								var mask = jQuerythis.attr('cmv_data-mask') || 'error...', mask_placeholder = jQuerythis.attr('cmv_data-mask-placeholder') || 'X';
								if (mask!=='error...')
								{
									jQuerythis.mask(mask, { placeholder : mask_placeholder});
									jQuerythis.removeAttr('cmv_data-mask');
								}
							}
					})
				
				CMV_J('[cmv_widgetclose_button]').click(function()
					{
				
						var cmv_result_id=CMV_J(this).attr("cmv_result_id");
						if(cmv_result_id==null || cmv_result_id===false) {cmv_result_id="";}
						if (cmv_result_id.length>0)
						{
							CMV_Result(cmv_result_id);
							if (!CMV_CallOrdered)
							{
								CMV_Call_Event("CMV_WidgetClose");
							}
						}
						CMV_J('[cmv_widgetbody]').remove();
						//CMV_J('body>*').each(function( index, element ) {CMV_J(element).removeClass('cmv_blur');})
				...
				CMV_J('[cmv_geo]').click(function(){
					CMV_J('[cmv_geo_menu]').show();
				})
				
				CMV_J('[cmv_geo_menu_item]').click(function(){
					var geo_text=CMV_J(this).text();
					CMV_J('[cmv_geo]').text(geo_text);
					CMV_J('[cmv_geo_menu]').hide();
			...
			CMV_J('[cmv_widgetphone_button]').click(function()
			{
				var attr1=CMV_J(this).attr("cmv_next_dialog");
				var cmv_start_call=CMV_J(this).attr("cmv_start_call");
				var cmv_result_id=CMV_J(this).attr("cmv_result_id");
				var cmv_start_delay_call=CMV_J(this).attr("cmv_start_delay_call");
				var cmv_stop_call=CMV_J(this).attr("cmv_stop_call");
				...
				if(cmv_start_call==="1")
				{
					var num1=CMV_J('[cmv_widgetphone_input]').val();
					if (num1&&(num1.length===12||num1.length===11))
						{
							CMV_MakeCall();
							if (attr1)
							{
							...
							{
								CMV_J('#cmv_input').hide();
							}
						}
				}
				
				else  if (cmv_stop_call==="1")
				{
					CMV_J.post(z_CMV_Server+"/Call/Widget/Do?zzType=StopCall",{WidgetID:cmv_widget_id,Cookie:cookie_val},function(data){});
					if (attr1)
					{
						CMV_GetCallWidget(attr1);
					}
				...
				else if (cmv_start_delay_call==="1")
				{
					var num1=CMV_J('[cmv_widgetphone_input]').val();
					if (num1&&num1.length===12)
						{
							CMV_MakeDelayCall(attr1);
						}
...
function CMV_MakeCall()
{
var callid='';
var num1=CMV_J('[cmv_widgetphone_input]').val();
if (num1&&(num1.length===12||num1.length===11))
{
				var counter = parseInt(CMV_J('[cmv_widgettimer]').attr("cmv_timer"));
				counter=counter*100;
				var timer_call=setInterval(function() {
								
								if (isNaN(counter)){	clearInterval(timer_call);}
								if (CMV_J('[cmv_widgettimer]').length==0){clearInterval(timer_call);}
								
								
                                counter--;
								CMV_J('[cmv_widgettimer]').attr("cmv_timer",counter);
                                if (counter >= 0) {
								
									var sec=  counter/100;
									
									var sec_text=""+counter;
									if (counter<100)
									{
									sec_text="00"+sec_text;
									if (counter<10)
										{
											sec_text="0"+sec_text;
										}
									...
									if (sec_text.length<4)
									{
									sec_text="0"+sec_text;
									
									}
									
									sec_text=sec_text.substring(0,2)+","+sec_text.substring(2);
									
									CMV_J('[cmv_widgettimer]').text(sec_text);
                                ...
                                if (counter === 0) {
									...
									if (CMV_J('[cmv_widget_notintime]').length>0)
									{
										CMV_J('[cmv_widget_notintime]').show();
									}
									else
									{
										var cmv_badid2=CMV_J('[cmv_widgettimer]').attr("cmv_badid");
										if (cmv_badid2){ CMV_GetCallWidget(cmv_badid2);}
									}
		...
		CMV_CallOrdered=true;
		var Geo="";
		if (CMV_J('[cmv_geo]').length>0)
		{
			Geo=CMV_J('[cmv_geo]').text();
		}
		else if (CMV_J('[CMV_Custom_GEO]').length>0)
		{
			Geo=CMV_J('[CMV_Custom_GEO]').text();
		}
		
		var url1=z_CMV_Server+"/Call/Widget/Do?zzType=MakeCall";
		CMV_J.post(url1,{GUID:cmv_trackerid,Phone:num1,Cookie:cookie_val,WidgetID:cmv_widget_id,Geo:Geo},function(data){
					
					if(data!=null && data!=="")
					{
							var callid=data;
							var url_callstatus=z_CMV_Server+"/Call/Widget/Do?zzType=Get_CallStatus";
							var timer3=	setInterval(function() {
								CMV_J.post(url_callstatus,{CALLID:callid,Cookie:cookie_val},function(data){
									if (data==="2")
									{
										clearInterval(timer3);
										
										var cmv_badid=CMV_J('[cmv_widgettimer]').attr("cmv_badid");
										if (cmv_badid){ CMV_GetCallWidget(cmv_badid); }
										
									}
									else if (data==="1")
									{
										clearInterval(timer3);		
										var cmv_goodid=CMV_J('[cmv_widgettimer]').attr("cmv_goodid");
										 if (cmv_goodid) {CMV_GetCallWidget(cmv_goodid);}
										CMV_Call_Event("CMV_Call");
									}
...
function CMV_MakeDelayCall(attr1)
{
...
var num1=CMV_J('[cmv_widgetphone_input]').val();
	if (num1&&num1.length===12)
	{
		...
		var cmv_delay_time=CMV_J('[cmv_widgetdelay_select = "1"]').val();
		
		CMV_Call_Event("CMV_CallBack");
		CMV_CallOrdered=true;
		var Geo="";
		if (CMV_J('[cmv_geo]').length>0)
		{
		...
		var url1=z_CMV_Server+"/Call/Widget/Do?zzType=MakeDelayCall";
		CMV_J.post(url1,{GUID:cmv_trackerid,Phone:num1,Cookie:cookie_val,WidgetID:cmv_widget_id,cmv_delay_time:cmv_delay_time,Geo:Geo},function(data){ CMV_GetCallWidget(attr1);})
...
function CMV_Result(ResultID)
{
		...
		if (CMV_J('[cmv_widgetphone_input]').length)
		{
			num1=CMV_J('[cmv_widgetphone_input]').val();
		}
		var cookie_val=CMV_readCookie("CMV_Z_Cookie");
		var  url_result=z_CMV_Server+"/Call/Widget/Do?zzType=Widget_Result";
		
		CMV_J.post(url_result,{WidgetID:cmv_widget_id,ResultID:ResultID,Phone:num1,Cookie:cookie_val},function(data){});
...
function CMV_Rating(Rate)
{
	var cookie_val=CMV_readCookie("CMV_Z_Cookie");
		...
		var  url_Rate=z_CMV_Server+"/Call/Widget/Do?zzType=Rating";
		CMV_J.post(url_Rate,{WidgetID:cmv_widget_id,Rate:Rate,Cookie:cookie_val},function(data){});


}

function CMV_Get_Call_Time()
{

	
	var addhourz=parseInt(CMV_J('[cmv_widgetdelay_select]').val());
	
	var currentDate=new Date(CMV_J.now());
	
	currentDate.setHours(currentDate.getHours()+addhourz);
	var mins1="" +currentDate.getMinutes();
	if (mins1.length==1)
	{
	mins1="0"+mins1;
	}
	
	var itog_date=currentDate.getHours()+":"+mins1;
	
	
	//CMV_J('.cmv_delay_call_time').text(itog_date);
	CMV_J('[cmv_widgetdelay_info]').text(itog_date);


}

function CMV_Log(text1)
{
if (Pars.A10=="1")
{
console.log(text1);
...
function CMV_Settings_Blur()
{
	var url_blur=z_CMV_Server+"/Call/Widget/Do?zzType=GetSettings_Blur";
	CMV_J.post(url_blur,{C1:Pars.C1,Time_move:v_timer_move,Time_Focus:v_timer_focus, TotalTime:v_total_timer,ScrollTotal:v_scroll_total,MoveTotal:b3},function(data)
				{
					v_timer_focus=0;
					v_timer_move=0;
					v_total_timer=0;
...
1)})}).promise().done(function(){e.onFormStyled.call()})}})(CMV_J);

var Pars;
CMV_J(document).ready(function() {
...
if (cmv_trackerid)
	{
		...
		var cookie_val=CMV_readCookie("CMV_Z_Cookie");
		var cookie_val2=CMV_readCookie("CMV_Z_Cookie_U");
		
		var url_GetSettings=z_CMV_Server+"/Call/Widget/Do?zzType=GetSettings";
		var z_resol=""+screen.width+" x "+ screen.height;
		CMV_J.post(url_GetSettings,{GUID:cmv_trackerid,Cookie:cookie_val,CookieU:cookie_val2,From:document.referrer,Title:CMV_J(document).find("title").text(),URL:window.location.href,resol:z_resol},function(data)
			{
				Pars = CMV_J.parseJSON(data);
				if (Pars.Active=="True"&&Pars.C2=="1")
				{		
						
						
						
						
						CMV_Log("C1="+Pars.C1+ " M1="+Pars.M1);
					if (Pars.Cookie!=""||Pars.cookie!=null)
						{
							if(cookie_val==null || cookie_val===false)
							{
								CMV_createCookie("CMV_Z_Cookie",Pars.Cookie,7);
								if(cookie_val2==null || cookie_val2===false) 
								{ CMV_createCookie("CMV_Z_Cookie_U",Pars.Cookie,365); }
							}
							
							CMV_J.post(z_CMV_Server+"/Call/Widget/Do?zzType=GetWidgetStyle",{C1:Pars.C1},function(data){
							var links = CMV_J.parseJSON(data);
								if (links.CS1)
								{
									CMV_J('body').append('<link rel= "stylesheet" type= "text/css" media= "screen" href= "'+links.CS1+'" > ');
								}
								if (links.CS2)
								{
									CMV_J('body').append('<link rel= "stylesheet" type= "text/css" media= "screen" href= "'+links.CS2+'" > ');
								}
								if (links.CS3)
								{
									CMV_J('body').append('<link rel= "stylesheet" type= "text/css" media= "screen" href= "'+links.CS3+'" > ');
								}
								if (links.CS4)
								{
									CMV_J('body').append('<link rel= "stylesheet" type= "text/css" media= "screen" href= "'+links.CS4+'" > ');
								}
							})
							/*
							CMV_J.post(z_CMV_Server+"/Call/Widget/Do?zzType=GetWidgetStyle",{GUID:cmv_trackerid},function(data){
								var link=data;
								CMV_J('body').append('<link rel= "stylesheet" type= "text/css" media= "screen" href= "'+link+'" > ');
								CMV_J('body').append('<link rel= "stylesheet" type= "text/css" media= "screen" href= "http://widget.cashmyvisit.com/css/cashmyvisit.css" > ');
							})
							*/
							
							CMV_J('.z_cmv_btn_call_now').click(function() {	CMV_GetCallWidget(0); })
							CMV_Start();
						}
				}
			})
